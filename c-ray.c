/*
 * c-ray.c: OpenMP version of a simple raytracing filter
 * (c)2020 Seiji Nishimura
 * $Id: c-ray.c,v 1.1.1.1 2020/07/31 00:00:00 seiji Exp seiji $
 */
...
void render(int xsz, int ysz, uint32_t *fb, int samples) {
	 ...
	 */
#pragma omp parallel for collapse(2) private(i,j,s) schedule(dynamic,1)
	for(j=0; j<ysz; j++) {
		for(i=0; i<xsz; i++) {
			uint32_t pixel;
			double r, g, b;
			...
			pixel = ((uint32_t)(MIN(r, 1.0) * 255.0) & 0xff) << RSHIFT |
